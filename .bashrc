...
for locald in $rconfd/*/ ; do
	...
	if [[ $locald/install.sh -nt $locald/.installed ]]; then
		echo "Installing module ${locald}..."
		source $locald/install.sh
		touch $locald/.installed
	fi
done

# sync reoconf modules
if [ -e $HOME/bin/rconf ]; then
	(echo $(date) > $rconfd/synclog &)
	(rconf start &>> $rconfd/synclog &)
fi
